...
**VALIDATE starts with the printer route** (ETAP 2e step 3): `getWorkflowFolderName(batch[0].printer)` from `createXML.js` — the SAME lookup `createXML` runs after the move — so a printer the shop profile cannot route is refused (`ERR_INVALID_PRINTER`) before any source file is stat'ed, locked or copied, not after the move at the cost of a rollback. Pinned by `createBatchPrinterGate.test.js`. It lives here and not in `submitBatch.js` because `submitBatchLabelGate.test.js` mocks `createXML.js` and `shopProfile.js` partially.
