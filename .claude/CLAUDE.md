...
- **CSS**: `.batch_group` is now nested inside `.day_body`. `.batch_group .card:nth-child(2)` / `:last-child` are descendant selectors, so they still hold — do NOT rewrite them as child selectors. `.day_header` is `position: sticky` inside `.cards_wrapper`; batch headers stay non-sticky on purpose. **The day is separated by an accent rule + type scale, not by a tonal step**: `--bg-grey-light` background, a 4px `border-left` in `--bg-black`, a 20px/700 date stacked over a second row with the counts and the stale pill, 54px tall. `.batch_group_header` keeps `--bg-grey`. Two earlier treatments were tried and rejected: a dark `--bg-black` bar (too heavy for this view) and a tonal inversion (day `--bg-grey` / batch white). Because the day is intentionally the LIGHTER of the two here, the separation rests entirely on the accent border, the type scale and the row height — if any of those is trimmed, the day header sinks into the list again, which was the original complaint.
