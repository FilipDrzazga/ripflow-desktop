...
- **CSS**: `.batch_group` is now nested inside `.day_body`. `.batch_group .card:nth-child(2)` / `:last-child` are descendant selectors, so they still hold — do NOT rewrite them as child selectors. `.day_header` is `position: sticky` inside `.cards_wrapper`; batch headers stay non-sticky on purpose. **Tonal order follows nesting order**: the day header takes `--bg-grey` + a 19px/700 date, and `.batch_group_header` was dropped from `--bg-grey` to `--bg-white` + a hairline `border-bottom` so it sits one step below. The day started out lighter than the batch header and visually sank beneath the rows nested inside it — never make the day the lighter of the two again. A dark (`--bg-black`) day bar was tried and rejected as too heavy for this view. The day carries its weight through **type size**, not a colour block.
